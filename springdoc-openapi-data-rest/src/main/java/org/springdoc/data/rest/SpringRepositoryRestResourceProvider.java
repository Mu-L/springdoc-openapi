...
public class SpringRepositoryRestResourceProvider implements RepositoryRestResourceProvider {
	 ...
	 */
	private static final String REPOSITORY_ENTITY_CONTROLLER = SPRING_DATA_REST_PACKAGE
			+ ".webmvc.RepositoryEntityController";
	 ...
	 */
	private static final String REPOSITORY_PROPERTY_CONTROLLER = SPRING_DATA_REST_PACKAGE
			+ ".webmvc.RepositoryPropertyReferenceController";

	/**
	 * The Delegating handler mapping class.
	 */
	private static final String DELEGATING_HANDLER_MAPPING_CLASS = SPRING_DATA_REST_PACKAGE
			+ ".webmvc.config.DelegatingHandlerMapping";

	/**
	 * The Delegating handler mapping interface.
	 */
	private static final String DELEGATING_HANDLER_MAPPING_INTERFACE = SPRING_DATA_REST_PACKAGE
			+ ".webmvc.support.DelegatingHandlerMapping";
	...
	public List<RouterOperation> getRouterOperations(OpenAPI openAPI) {
		...
		for (Class<?> domainType : repositories) {
			...
			if (resourceMetadata.isExported()) {
				for (HandlerMapping handlerMapping : handlerMappingList) {
					if (handlerMapping instanceof RepositoryRestHandlerMapping) {
						...
						Map<RequestMappingInfo, HandlerMethod> handlerMethodMap = repositoryRestHandlerMapping
								.getHandlerMethods();
						// Entity controllers lookup first
						Map<RequestMappingInfo, HandlerMethod> handlerMethodMapFiltered = handlerMethodMap.entrySet()
								.stream()
								.filter(requestMappingInfoHandlerMethodEntry -> REPOSITORY_ENTITY_CONTROLLER.equals(
										requestMappingInfoHandlerMethodEntry.getValue().getBeanType().getName()))
								.filter(controller -> !AbstractOpenApiResource
										.isHiddenRestControllers(controller.getValue().getBeanType()))
								.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a1, a2) -> a1));
						dataRestRepository.setControllerType(ControllerType.ENTITY);
						routerOperationList.addAll(findControllers(handlerMethodMapFiltered, resourceMetadata,
								dataRestRepository, openAPI));

						Map<RequestMappingInfo, HandlerMethod> handlerMethodMapFilteredMethodMap = handlerMethodMap
								.entrySet().stream()
								.filter(requestMappingInfoHandlerMethodEntry -> REPOSITORY_PROPERTY_CONTROLLER.equals(
										requestMappingInfoHandlerMethodEntry.getValue().getBeanType().getName()))
								.filter(controller -> !AbstractOpenApiResource
										.isHiddenRestControllers(controller.getValue().getBeanType()))
								.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a1, a2) -> a1));
						...
						entity.doWithAssociations((SimpleAssociationHandler) association -> {
							...
							if (jackson.isExported(property) && associations.isLinkableAssociation(property)) {
								dataRestRepository
										.setRelationName(resourceMetadata.getMappingFor(property).getRel().value());
								dataRestRepository.setControllerType(ControllerType.PROPERTY);
								routerOperationList.addAll(findControllers(handlerMethodMapFilteredMethodMap,
										resourceMetadata, dataRestRepository, openAPI));
							}
						});
					}
						...
						Map<RequestMappingInfo, HandlerMethod> handlerMethodMap = beanBasePathAwareHandlerMapping
								.getHandlerMethods();
						Map<RequestMappingInfo, HandlerMethod> handlerMethodMapFiltered = handlerMethodMap.entrySet()
								.stream()
								.filter(requestMappingInfoHandlerMethodEntry -> REPOSITORY_SCHEMA_CONTROLLER.equals(
										requestMappingInfoHandlerMethodEntry.getValue().getBeanType().getName()))
								.filter(controller -> !AbstractOpenApiResource
										.isHiddenRestControllers(controller.getValue().getBeanType()))
								.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a1, a2) -> a1));
			...
			// search
			routerOperationList.addAll(
					findSearchResourceMappings(openAPI, handlerMappingList, dataRestRepository, resourceMetadata));
		}
		routerOperationList.addAll(findRepositoryRestControllers(handlerMappingList, openAPI));
	 ...
	 * @return the list
	 */
	private List<RouterOperation> findSearchResourceMappings(OpenAPI openAPI, List<HandlerMapping> handlerMappingList,
		...
		for (HandlerMapping handlerMapping : handlerMappingList) {
			if (handlerMapping instanceof RepositoryRestHandlerMapping) {
				...
				Map<RequestMappingInfo, HandlerMethod> handlerMethodMapFiltered = handlerMethodMap.entrySet().stream()
						.filter(requestMappingInfoHandlerMethodEntry -> REPOSITORY_SEARCH_CONTROLLER
								.equals(requestMappingInfoHandlerMethodEntry.getValue().getBeanType().getName()))
						.filter(controller -> !AbstractOpenApiResource
								.isHiddenRestControllers(controller.getValue().getBeanType()))
						.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a1, a2) -> a1));
				...
				if (searchResourceMappings.isExported()) {
					routerOperationList.addAll(findSearchControllers(handlerMethodMapFiltered, resourceMetadata,
							dataRestRepository, openAPI, searchResourceMappings));
				}
			}
		}
		return routerOperationList;
	 ...
	 * @return the list
	 */
	private List<RouterOperation> findSearchControllers(Map<RequestMappingInfo, HandlerMethod> handlerMethodMap,
		...
		methodResourceMappingStream.forEach(methodResourceMapping -> routerOperationList
				.addAll(dataRestRouterOperationService.buildSearchRouterOperationList(handlerMethodMap,
						resourceMetadata, dataRestRepository, openAPI, methodResourceMapping)));
		return routerOperationList;
	}
	 ...
	 * @return the list
	 */
	private List<RouterOperation> findControllers(Map<RequestMappingInfo, HandlerMethod> handlerMethodMap,
			ResourceMetadata resourceMetadata, DataRestRepository dataRestRepository, OpenAPI openAPI) {
		return dataRestRouterOperationService.buildEntityRouterOperationList(handlerMethodMap, resourceMetadata,
				dataRestRepository, openAPI);
	}

	/**
	 * Find repository rest controllers.
	 * @param openAPI the open api
	 * @param handlerMappingList the handler mapping list
	 * @return the list
	 */
	private List<RouterOperation> findRepositoryRestControllers(List<HandlerMapping> handlerMappingList,
			OpenAPI openAPI) {
		return handlerMappingList.stream().filter(RequestMappingInfoHandlerMapping.class::isInstance)
				...
				.filter(entry -> !entry.getValue().getBeanType().getName().startsWith(SPRING_DATA_REST_PACKAGE))
				.flatMap(entry -> {
					ResourceMetadata resourceMetadata = mappings
							.getMetadataFor(entry.getValue().getReturnType().getParameterType());
					return dataRestRouterOperationService
							.buildCustomRouterOperationList(entry.getKey(), entry.getValue(), resourceMetadata, openAPI)
							.stream();
